...
const COLORS = [
  'var(--accent-violet)', 'var(--accent-cyan)', 'var(--accent-emerald)',
  'var(--accent-rose)', '#f59e0b', '#3b82f6', '#a855f7', '#14b8a6',
];
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Детальний аналіз ваших витрат</p>
          </div>
        </div>
              ...
              <div key={currency} className="glass-card p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold" style={{ color: 'var(--accent-rose)' }}>-{total.toFixed(2)} {symbol}</p>
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    {sorted.slice(0, 4).map(([cat, amt], i) => (
                      <div key={cat} className="h-2 rounded-full" style={{ width: `${total > 0 ? (amt / total) * 120 : 0}px`, background: COLORS[i % COLORS.length], minWidth: 4 }} />
                    ))}
                  </div>
                </div>
