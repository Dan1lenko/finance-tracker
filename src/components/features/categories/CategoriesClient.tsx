...
const COLORS = [
  '#3451D1', '#22A99D', '#D45A38', '#8a6a30',
  '#5a6ee0', '#3dd4c8', '#e07a5a', '#c89a5a',
];
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl flex items-center gap-2" style={{ color: 'var(--color-text)', fontWeight: 500 }}>
              <PieChart className="w-6 h-6" style={{ color: 'var(--color-income)' }} /> Витрати за категоріями
            </h1>
        ...
        {!hasData ? (
          <div className="glass-card py-20 text-center">
            <TrendingDown className="w-12 h-12 mx-auto mb-4" style={{ color: 'var(--color-surface-2)' }} />
              ...
              <div key={currency} className="glass-card p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl" style={{ color: 'var(--color-expense-text)', fontWeight: 500 }}>-{total.toFixed(2)} {symbol}</p>
                  ...
                  <div className="flex flex-col gap-1 items-end">
                    {sorted.slice(0, 4).map(([cat, amt], i) => (
                      <div key={cat} className="h-2 rounded-full"
                        style={{ width: `${total > 0 ? (amt / total) * 120 : 0}px`, background: COLORS[i % COLORS.length], minWidth: 4 }} />
                    ))}
                  </div>
                </div>
                <div style={{ borderTop: '0.5px solid var(--color-surface-2)' }} />
