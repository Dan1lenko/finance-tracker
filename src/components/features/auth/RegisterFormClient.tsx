...
export default function RegisterFormClient() {
  ...
  const validate = () => {
    ...
    else if (name.trim().length < 2) newErrors.name = "Ім'я має бути не менше 2 символів";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/3 -right-32 w-96 h-96 rounded-full opacity-45"
          style={{ background: 'var(--color-income-bg)', filter: 'blur(90px)' }} />
        <div className="absolute bottom-1/3 -left-32 w-96 h-96 rounded-full opacity-45"
      ...
      <div className="relative w-full max-w-md animate-fade-in-up">
        <div className="glass-card p-6 sm:p-8">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl" style={{ color: 'var(--color-text)', fontWeight: 500 }}>Реєстрація</h2>
            <p className="mt-2 text-sm" style={{ color: 'var(--color-muted)' }}>
              Вже маєте акаунт?{' '}
              <Link href="/auth/login" className="font-medium transition-colors" style={{ color: 'var(--color-primary)' }}>Увійти</Link>
            </p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleRegister} noValidate>
            <div>
              <div className="relative">
                ...
                <input type="text" className={`glass-input ${errors.name ? 'input-error' : ''}`} style={{ paddingLeft: '2.5rem' }}
                  placeholder="Ім'я" value={name}
                  onChange={(e) => { setName(e.target.value); clearError('name'); }} autoComplete="name" />
              </div>
            ...
            <div>
              <div className="relative">
                ...
                <input type="password" className={`glass-input ${errors.password ? 'input-error' : ''}`} style={{ paddingLeft: '2.5rem' }}
                  placeholder="Пароль (мінімум 6 символів)" value={password}
                  onChange={(e) => { setPassword(e.target.value); clearError('password'); }} autoComplete="new-password" />
              </div>
